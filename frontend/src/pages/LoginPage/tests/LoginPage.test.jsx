...
test("User can log in", async () => {
    ...
    const userProfileLink = await screen.findByRole("link", { name: /James Smith/i, });
...
test("User cannot log in", async () => {
    ...
    expect(passwordField).toBeInTheDocument();
    let loginButton = await screen.findByRole("button", { name: /log in/i });
    expect(loginButton).toBeInTheDocument();
    expect(loginButton).toBeDisabled();
    ...
    emailField = await screen.findByRole("textbox", { name: /email address/i });
    expect(emailField).toBeInTheDocument();
